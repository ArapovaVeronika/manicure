...
new Swiper('.work__swiper',{
    effect: 'cube',
    autoplay:{
        delay:2000, //задержка перед сменой  слайда
        stopOnlastSlide: false, //отключили остановку на последнем слайде
        disableOnInteraction: false, //отключаем остановку при ручном перелистывании
    },
    loop:true,
     speed: 1500, //скорость перехода слайда
...
new Swiper('#reviewsSwiper',{
    effect: 'flip',
    loop:true, //бесконечная прокрутка слайдов (барабан)
     speed: 1500,
     navigation: { //добавляем стрелки
...
function formSubmit(){
    ...
    iframe.onload = function(){
        ...
        setTimeout(() => {    //функция запускается с нужной нам задержкой
            document.forms.order.reset(); // форма очищается
